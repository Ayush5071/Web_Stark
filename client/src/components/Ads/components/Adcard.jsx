...
const AdCard = ({ ad, onLike }) => {
  ...
  return (
    <div className="max-w-sm p-6 bg-white text-black rounded-lg shadow-lg relative hover:shadow-xl transition-shadow duration-300">
      <img
        src={ad.imageurl}
        alt={ad.title}
      ...
      />
      <div>
        <h3 className="text-2xl font-semibold text-gray-900">{ad.title}</h3>
      ...
      <div className="absolute top-2 right-2 bg-green-600 px-2 py-1 text-xs text-white rounded-full">
        {ad.status}
      </div>
    </div>
  );
